...
final class PreviewRenderTest extends TestCase {
	...
	public function test_preview_defaults_to_home_and_routes_by_page_param(): void {
		require_once dirname( __DIR__, 2 ) . '/preview/index.php';
		$this->assertStringContainsString( 'class="ch-home-hero"', Blueworx_Clubhouse_Test_Site::slug( '' ) );

		// A known page (about) renders its own markup rather than Home's.
		$this->assertStringContainsString( 'class="ch-nav"', Blueworx_Clubhouse_Test_Site::slug( 'about' ) );
	}

	public function test_preview_routes_the_four_collection_pages(): void {
		require_once dirname( __DIR__, 2 ) . '/preview/index.php';
		$this->assertStringContainsString( 'ch-scards', Blueworx_Clubhouse_Test_Site::slug( 'sports' ) );
		$this->assertStringContainsString( 'ch-scards', Blueworx_Clubhouse_Test_Site::slug( 'teams' ) );
		$this->assertStringContainsString( 'ch-events', Blueworx_Clubhouse_Test_Site::slug( 'events' ) );
		$this->assertStringContainsString( 'ch-cal__month', Blueworx_Clubhouse_Test_Site::slug( 'calendar' ) );
	}
