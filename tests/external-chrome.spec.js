...
test('a page another plugin owns gets the club header and footer', async ({ page }) => {
...
test("the other plugin's content is kept, inside the content well", async ({ page }) => {
  await page.goto(FIXTURE);
...
test('the look stylesheet loads so the page is in the club typeface', async ({ page }) => {
  await page.goto(FIXTURE);
...
test('the scroll reveal is not loaded, and nothing is hidden by us', async ({ page }) => {
  await page.goto(FIXTURE);
...
test('the chrome is emitted once, not once per nested body match', async ({ page }) => {
  await page.goto(FIXTURE);
