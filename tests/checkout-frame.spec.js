...
// for it. What these assert is the frame — BlueWorx Labs draws it now, with
// the club's own footer links hooked in — and the shop's content surviving
...
test('the field theme is asked for in the head, not the footer @wordpress', async ({ page }) => {
  ...
  // Labs' field theme for SureCart's form, not ClubHouse's old surecart.css.
  await page.goto(CHECKOUT);
  await expect(
    page.locator('head link[rel="stylesheet"][href*="store-surecart.css"]')
  ).toHaveCount(1);
});
...
test('the checkout owns the whole page, with no theme chrome around it @wordpress', async ({ page }) => {
  ...
  await expect(page.locator('footer')).toHaveClass(/blueworx-checkout__foot/);
});
...
test('nothing renders below the checkout footer @wordpress', async ({ page }) => {
  ...
  const slack = await page.evaluate(() => {
    const foot = document.querySelector('.blueworx-checkout__foot').getBoundingClientRect();
