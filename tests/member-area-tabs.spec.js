...
test('the member area draws the sidebar and the top bar @wordpress', async ({ page }) => {
  ...
  await expect(page.locator('.blueworx-store__side')).toHaveCount(1);
  await expect(page.locator('.blueworx-store__side .bw-secnav')).toHaveCount(1);
  await expect(page.locator('.blueworx-store__main .bw-pagehead')).toHaveCount(1);
});

test('every panel is on the page, with one shown @wordpress', async ({ page }) => {
  await page.goto('/member-dashboard/');
  const panels = page.locator('.blueworx-store__panel');
  // LatePoint is not installed, so Bookings is not among them.
  await expect(panels).not.toHaveCount(0);
  await expect(page.locator('.blueworx-store__panel:not([hidden])')).toHaveCount(1);
});
...
test('clicking a nav item swaps panels without navigating @wordpress', async ({ page }) => {
  await page.route('**/member-dashboard/', async (route) => {
    ...
    body = body.replace(
      '</main>',
      '<div class="blueworx-store__panel" data-view="test-second-view" hidden>Injected second panel</div></main>'
    );
  ...
  const firstPanel = page.locator('.blueworx-store__panel').first();
...
test('a phone gets the bottom bar, not the sidebar nav @wordpress', async ({ page }) => {
  await page.route('**/member-dashboard/', async (route) => {
    ...
    const extraNavItems = ['bookings', 'orders', 'invoices', 'plans']
      .map((key) => '<a class="bw-secnav__item" data-view-link="' + key + '"'
        ...
        + '<span class="blueworx-store__navlabel">' + key + '</span></a>')
      .join('');
    ...
    const barOpen = '<nav class="blueworx-store__tabbar" aria-label="Your account">';
    const extraTabs = ['bookings']
      .map((key) => '<a class="blueworx-store__tab" data-view-link="' + key + '"'
        + ' data-view-title="' + key + '" data-view-lede=""'
        + ' href="/member-dashboard/?view=' + key + '">'
        + '<span class="blueworx-store__tablabel">' + key + '</span></a>')
      .join('');
  ...
  await expect(page.locator('.blueworx-store__tabbar')).toBeVisible();
  // Dashboard, Billing, Account, the spliced-in Bookings, and the way back.
  await expect(page.locator('.blueworx-store__tab')).toHaveCount(5);
  await expect(page.locator('.blueworx-store__side .bw-secnav')).toBeHidden();

  // Billing carries the Plans, Orders and Invoices panels itself, so no link
  // rows are drawn under it — or under any other screen.
  await expect(page.locator('.blueworx-store__more')).toHaveCount(0);
  await page.locator('.blueworx-store__tab[data-view-link="billing"]').click();
  await expect(page.locator('.blueworx-store__more')).toHaveCount(0);
  ...
  await expect(page.locator('.blueworx-store__side .blueworx-store__back')).toBeHidden();
  ...
  await expect(page.locator('.blueworx-store__tabbar')).toBeHidden();
  await expect(page.locator('.blueworx-store__side .bw-secnav')).toBeVisible();
  await expect(page.locator('.blueworx-store__side .blueworx-store__back')).toBeVisible();
});
...
test('the bar shows exactly the curated list, not every view @wordpress', async ({ page }) => {
  ...
  await expect(page.locator('.blueworx-store__tabbar')).toBeVisible();
  const tabs = page.locator('.blueworx-store__tab');
  ...
  for (const key of ['orders', 'invoices', 'plans']) {
    await expect(page.locator(`.blueworx-store__tab[data-view-link="${key}"]`)).toHaveCount(0);
  }
...
// The page head (.blueworx-store__head) is dropped on a phone; its title,
...
test('a phone drops the page head in favour of the top row, the desktop keeps it @wordpress', async ({ page }) => {
  ...
  await expect(page.locator('.blueworx-store__head')).toBeHidden();
  await expect(page.locator('.blueworx-store__viewtext')).toBeVisible();
  await expect(page.locator('.blueworx-store__viewtitle')).toHaveText('Your account');
  await expect(page.locator('.blueworx-store__brandsignout')).toBeVisible();
  await expect(page.locator('.blueworx-store__tab').last()).toContainText('Back home');
  ...
  await page.setViewportSize({ width: 1280, height: 900 });
  await expect(page.locator('.blueworx-store__head')).toBeVisible();
  await expect(page.locator('.blueworx-store__viewtext')).toBeHidden();
});
...
test('a phone does not show who is signed in @wordpress', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/member-dashboard/');
  await expect(page.locator('.blueworx-store__person')).toBeHidden();

  await page.setViewportSize({ width: 1280, height: 900 });
  await expect(page.locator('.blueworx-store__person')).toBeVisible();
});
