...
/**
 * The header menu's save. It has no screen of its own — the builder is a tab on
 * Clubhouse Setup — so this is a controller with one job: turn a posted menu
 * form into a stored tree.
 *
 * It used to live on the Club Pages controller, which was where the builder
 * used to be. Club Pages has gone; the menu has not, and neither has the nonce
 * its form posts, so an owner with the page already open still saves cleanly.
 *
 * Pure enough to test without WordPress: handle_save takes a Storage.
 *
 * @package BlueworxLabsClubhouse
 */
final class Blueworx_Clubhouse_Menu_Controller {

	// edit_clubhouse_content — owner, content editor and admin. Arranging the
	// menu is the Content Editor's job, and manage_clubhouse is the one
	// capability that role does not hold.
	...
	public static function handle_save( array $post, Blueworx_Clubhouse_Storage $storage ): array {
		if ( 'menu' !== (string) ( $post['clubhouse_content_tab'] ?? '' ) ) {
			return array();
		}
		if ( ! array_key_exists( 'menu', $post ) ) {
			// PHP's max_input_vars can truncate a large POST before the 'menu' key
			// ever arrives, and a wholly-absent key must not be read as "every row
			// was deleted" — Menu::tree() treats a stored empty array as exactly
			// that. An explicitly submitted empty array (the owner really did empty
			// it) still reaches menu_from_post() below and saves as empty.
			return array();
		}
